...
export class ReviewEngine {
  ...
  // 行動前檢查 - 檢索相關歷史錯誤
  async checkBeforeAction(action: string, context?: string): Promise<Prevention[]> {
    // 搜尋關鍵字
    const keywords = this.extractKeywords(action + ' ' + (context || ''));
    const results: Prevention[] = [];

    for (const keyword of keywords) {
      const errors = await this.notion.searchErrors(keyword);
      
      for (const page of errors) {
        const titleProp = page.properties['標題'] as { title: { text: { content: string } }[] };
        const preventionProp = page.properties['預防措施'] as { rich_text: { text: { content: string } }[] };
        ...
        results.push({
          ...
          severity: (severityProp?.select?.name?.toLowerCase() as any) || 'medium'
        });
      }
    }
  ...
  async getDailyReview(): Promise<{
    ...
    const trend: 'improving' | 'stable' | 'worsening' = 
      stats.open > stats.resolved ? 'worsening' : 

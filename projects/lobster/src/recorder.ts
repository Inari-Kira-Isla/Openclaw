...
export class Recorder {
  ...
  // 記錄錯誤
  async recordError(error: Omit<LobsterError, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    // 先檢查是否已經存在類似的錯誤
    ...
    if (existing.length > 0) {
      ...
      const occurrenceProp = page.properties['發生次數'] as { number: number };
  ...
  // 從偵測結果記錄錯誤
  async recordFromDetection(detection: DetectionResult): Promise<string> {
    const error: Omit<LobsterError, 'id' | 'createdAt' | 'updatedAt'> = {
  ...
  // 記錄成功
  async recordSuccess(success: Omit<SuccessRecord, 'id' | 'createdAt'>): Promise<string> {
  ...
  // 解決錯誤
  async resolveError(errorId: string, solution: string, prevention: string): Promise<void> {
    await this.notion.updateErrorStatus(errorId, 'Resolved');
    console.log(`Error resolved: ${errorId}`);
  }

  // 獲取開放的錯誤
  async getOpenErrors(): Promise<void> {

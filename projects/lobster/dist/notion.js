...
class NotionClient {
    constructor(config) {
        this.client = new client_1.Client({ auth: config.notionApiKey });
        this.config = config;
    }
    // Create error record
    async createErrorRecord(error) {
    ...
    async createSuccessRecord(success) {
        const response = await this.client.pages.create({
    ...
    // Update error status
    async updateErrorStatus(pageId, status) {
    ...
    async getErrorStats() {
        ...
        const pages = allErrors.results;
        let open = 0;
        let resolved = 0;
        const bySeverity = { high: 0, medium: 0, low: 0 };
        for (const page of pages) {
            const statusProp = page.properties['狀態'];
            const severityProp = page.properties['嚴重程度'];
            if (statusProp?.select?.name === 'Resolved')
                resolved++;
            else
                open++;
